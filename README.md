...
`memobird_local.print_image` prints an image from a file, a URL, a camera snapshot or SVG markup (use exactly one). The image is scaled to the paper width (384 dots) and converted to black and white; tall images are fine.
...
`svg` takes SVG markup, for example line art generated by an AI model through `ai_task.generate_data`. It is rendered by a built-in pure-Python rasterizer (Home Assistant has no cairo): paths with all commands including arcs, circles, ellipses, rectangles, lines, polylines, polygons, groups and transforms, fills (`fill-rule` included) and strokes. Text, gradients, filters, masks and `<use>` are ignored. SVG is printed without dithering unless `dither: true`.
